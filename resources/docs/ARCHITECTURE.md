...
- **`<PlannerTab>`**: Encapsulates the main drag-and-drop planned grid, levels/talents transition rows, power standby states, and modal trigger props. Includes a **"Ready to Upgrade" filter bar** (`.planner-filter-bar`) above the cards grid that allows users to filter between *Show All*, *Highlight Ready* (green glow on ready cards, others dimmed), and *Only Show Ready* (hides all non-ready and completed cards). Filtering is purely render-level — it does not affect sequential inventory allocation or SummaryPanel totals. Styled by `PlannerTab.css`.
...
- **Ready to Upgrade Render Filter**: `PlannerTab.tsx` exposes a three-state `readyFilter` (`'all' | 'highlight' | 'readyOnly'`) that defaults to `'all'` (disabled). A `getPlanStatus(planned)` helper determines `isDone` (requirements list is empty) and `isReady` (card is enabled, not done, and all requirements have `isEnough === true`). A derived `visiblePlannedItems` array gates which cards enter the JSX map: in any non-`'all'` mode, completed cards are hidden; in `'readyOnly'` mode, only `isReady` cards are shown. In `'highlight'` mode, ready cards receive an inline green glowing border (`rgba(129, 199, 132, 0.85)`) and `scale(1.01)`, while non-ready cards receive a heavy grayscale/opacity dim (`grayscale(0.9) opacity(0.2)`). The `SummaryPanel` and sequential inventory allocation are **not** affected by this filter — the full `plannedItems` array continues to drive all calculations.
