...
- **Planner Priority Reordering Systems**: Changing progression weight priority is supported in two ways for a mixed list of characters and weapons:
  1. A standalone visual **Priority Manager Modal** featuring immediate visual drag-swaps of item rows while preserving original saved order number badges next to them until save confirmation. The modal adapts to the plan count with an **adaptive multi-column layout** (≤30 items → 10 rows/column, 30–60 items → 15 rows/column, >60 items → single scrollable column), and modal width scales to `280px × numCols + 60px`. Priority numbers are derived by looking up the `id` field using the same construction key (`p.id || \`character:${p.key}\` || \`weapon:${p.weaponIndex}\``) in `savedOrder`, which eliminates the character-position-always-0 bug that occurred when using bare name keys.
...
- **Disabled Card Material Visibility**: When a planner card is toggled off (standby/disabled), its required materials are still visible — calculated by passing a `null` inventory to `calculateRequirements`, which simulates 0 owned items. A grey **Disabled** badge appears in the card header. Material icons on disabled cards remain clickable and can open the Quick Inventory modal to make fast edits.
- **Ready to Upgrade Filter**: A three-state filter bar (`.planner-filter-bar`) sits above the planner cards grid. Options are *Show All* (default, no filtering), *Highlight Ready* (ready-to-upgrade cards glow green with a `rgba(129, 199, 132, 0.85)` border and `scale(1.01)`; others are dimmed to `grayscale(0.9) opacity(0.2)`; completed cards are hidden), and *Only Show Ready* (only active, fully-satisfiable plans are displayed; completed and in-progress cards are hidden). The filter state (`readyFilter`) is purely presentational — the `SummaryPanel` totals and sequential inventory allocation are always computed from the full unfiltered `plannedItems` array regardless of active filter mode.
...
- **Data Scraping & Update Automation**: The project uses custom scripts (`resources/scripts/`) to extract data from `genshin-db` and download game assets. The unified coordinator script `resources/scripts/updateData.cjs` (run via `npm run update-data`) regenerates all maps (`materialMap.json`, `characterMap.json`, `weaponMap.json`, `weaponRequirementsMap.json`) and downloads new character/weapon assets in one step. The generator scripts also extract a `version` field (e.g. `"7.0"`) from each entry for release-date sorting.
  - **Patch Override System**: When `genshin-db` lags behind a live game patch (e.g. new 7.0 characters/weapons not yet in the package), override entries can be added directly to `src/maps/patches.json`. The `resources/scripts/applyPatches.cjs` script deep-merges these overrides into the final JSON maps and downloads the corresponding CDN assets (icons, weapon PNGs, artifact PNGs) automatically. This is the preferred escape hatch rather than hardcoding anything in `App.tsx`. The helper `resources/scripts/fetchYattaAssets.cjs` can be used to manually pull exact Yatta CDN assets during patch research.
  - If data is missing or outdated after a game update, run `npm run update-data` first; if the new content is not yet in `genshin-db`, add it to `patches.json` and run `node resources/scripts/applyPatches.cjs`.
...
- **Planner-Only Quick Inventory Modal**:
  ...
  * Number inputs (Inventory and Add/Subtract) auto-select their content on focus via `onFocus={(e) => e.target.select()}`, allowing immediate keyboard entry without manual clearing.
