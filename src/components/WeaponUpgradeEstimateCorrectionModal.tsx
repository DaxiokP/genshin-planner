...
import { calculateRemainingOres } from '../utils/upgradeHelpers';
...
interface WeaponUpgradeEstimateCorrectionModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const WeaponUpgradeEstimateCorrectionModal: React.FC<WeaponUpgradeEstimateCorrectionModalProps> = ({
  isOpen,
  onClose,
  materials,
  estimatedSpend,
  ...
  const [mysticInput, setMysticInput] = useState(0);
  ...
  useEffect(() => {
    if (isOpen && materials) {
      ...
      const currentMystic = getOwnedCount('mysticenhancementore');
      const currentFine = getOwnedCount('fineenhancementore');
      const currentNormal = getOwnedCount('enhancementore');

      setMoraInput(Math.max(0, currentMora - estimatedSpend.mora));

      const remainingOres = calculateRemainingOres(
        estimatedSpend.mysticenhancementore,
        currentMystic,
        currentFine,
        currentNormal
      );

      setMysticInput(remainingOres.mysticenhancementore);
      setFineInput(remainingOres.fineenhancementore);
      setNormalInput(remainingOres.enhancementore);
    }
  ...
  const handleConfirm = () => {
    onConfirm(moraInput, {
      mysticenhancementore: mysticInput,
      fineenhancementore: fineInput,
      enhancementore: normalInput,
    });
  };
      ...
      >
        ...
        <div className="modal-content" style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
          <p style={{ margin: 0, fontSize: '0.88rem', color: 'rgba(255, 255, 255, 0.75)', lineHeight: 1.5 }}>
            We estimated your leftover Mora and weapon enhancement ores based on your inventory. Make any corrections below if needed.
          </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
            ...
            >
              <div 
                className={`bg-rarity-${getMaterialRarity('mysticenhancementore')}`}
                style={{
                  width: '40px',
                  height: '40px',
                  borderRadius: '6px',
              ...
              >
                <img src={getMaterialIconSrc('mysticenhancementore')} alt="Mystic Enhancement Ore" style={{ width: '80%', height: '80%', objectFit: 'contain' }} />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '0.75rem', color: 'rgba(255, 255, 255, 0.4)', fontWeight: 'bold' }}>
                  {getMaterialName('mysticenhancementore')}
                </div>
                <div style={{ fontSize: '0.85rem', color: '#ffb84d', fontWeight: 'bold' }}>
                  Spend Estimate: -{estimatedSpend.mysticenhancementore}
                </div>
              </div>
              <input
                type="number"
                ...
                value={mysticInput === 0 ? '' : mysticInput}
                placeholder="0"
                onChange={e => {
                  const v = parseInt(e.target.value, 10);
                  setMysticInput(isNaN(v) ? 0 : Math.max(0, v));
                }}
              />
            </div>
            ...
            >
              <div 
                className={`bg-rarity-${getMaterialRarity('fineenhancementore')}`}
                style={{
                  width: '40px',
                  height: '40px',
                  borderRadius: '6px',
              ...
              >
                <img src={getMaterialIconSrc('fineenhancementore')} alt="Fine Enhancement Ore" style={{ width: '80%', height: '80%', objectFit: 'contain' }} />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '0.75rem', color: 'rgba(255, 255, 255, 0.4)', fontWeight: 'bold' }}>
                  {getMaterialName('fineenhancementore')}
                </div>
              ...
              <input
                ...
                onChange={e => {
                  const v = parseInt(e.target.value, 10);
                  setFineInput(isNaN(v) ? 0 : Math.max(0, v));
                }}
              />
            </div>
            ...
            >
              <div 
                className={`bg-rarity-${getMaterialRarity('enhancementore')}`}
                style={{
                  width: '40px',
                  height: '40px',
                  borderRadius: '6px',
              ...
              >
                <img src={getMaterialIconSrc('enhancementore')} alt="Enhancement Ore" style={{ width: '80%', height: '80%', objectFit: 'contain' }} />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '0.75rem', color: 'rgba(255, 255, 255, 0.4)', fontWeight: 'bold' }}>
                  {getMaterialName('enhancementore')}
                </div>
              ...
              <input
                ...
                value={normalInput === 0 ? '' : normalInput}
                placeholder="0"
                onChange={e => {
                  const v = parseInt(e.target.value, 10);
                  setNormalInput(isNaN(v) ? 0 : Math.max(0, v));
                }}
              />
            </div>
