...
- An ascension spark symbol (`✦`) represents an ascended cap (e.g. `80✦` or `90✦`).
...
- **Spread Values**: Formatted as `[Current]   ➔   [Desired]` (e.g. `20   ➔   90✦` or `1   ➔   10`).
- **Ascension Indicator**: The ascension spark `✦` is rendered inline next to levels (e.g. `80✦`) to ensure it travels correctly with the level values and never clips the card border.
...
- **Quantities Representation**:
  ...
  - **Mora & EXP Books**: Displays as a estimated required value prefixed by a tilde (e.g., `~1.2M` or `~415`), bypassing owned/needed calculations for visual simplicity.
