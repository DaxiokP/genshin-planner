...
- ⚡ **Planner-Only Quick Inventory Modal**: Open a compact inventory modal directly by clicking any material icon on planner cards or summary tiles. Updates inventory instantly without navigating away from the planner page. Number inputs auto-select their content on focus for instant overriding.
- 🔄 **Automatic GOOD Import Sync**: When importing a new GOOD file, the planner automatically reconciles your planned items — advancing their current levels and talents to match the new import data without duplicating or resetting existing plans.
- ✅ **Show Done Filter**: The character selection modal hides already-completed plans by default. Toggle "Show Done" to reveal them dimmed with a DONE badge, keeping the view focused on what still needs farming.
- 📋 **Disabled Card Material Preview**: Planner cards toggled to standby (disabled) still display their full required material list — calculated against a zero inventory so you can see what would be needed. A grey **Disabled** badge marks the card's status, and material icons remain clickable for quick inventory edits.
- 👥 **Multi-Profile Shared Accounts & Account Settings**: Dynamically create, swap, rename, and manage multiple planners under a single account via the dedicated **Account Settings** page. Quickly switch profiles from the header dropdown in real time.
